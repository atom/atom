...
module.exports = ({commandRegistry, commandInstaller, config, notificationManager, project, clipboard}) ->
  ...
  commandRegistry.add(
    'atom-pane',
    {
      ...
      'pane:move-to-very-top': -> @getModel().moveToVeryTop()
      'pane:move-to-very-bottom': -> @getModel().moveToVeryBottom()
      'pane:move-to-very-left': -> @getModel().moveToVeryLeft()
      'pane:move-to-very-right': -> @getModel().moveToVeryRight()
