...
module.exports = function(task) {
  ...
  return runPackager({
    ...
    if (process.platform === 'darwin') {
      ...
      setAtomHelperVersion(packagedAppPath, task);
    } else if (process.platform === 'linux') {
      bundledResourcesPath = path.join(packagedAppPath, 'resources');
      chmodNodeFiles(packagedAppPath, task);
    } else {
      bundledResourcesPath = path.join(packagedAppPath, 'resources');
    }

    return copyNonASARResources(
      packagedAppPath,
      bundledResourcesPath,
      task
    ).then(() => {
      task.log(`Application bundle created at ${packagedAppPath}`);
      task.done();
...
function copyNonASARResources(packagedAppPath, bundledResourcesPath, task) {
  task.log(`Copying non-ASAR resources to ${bundledResourcesPath}`);
  fs.copySync(
    path.join(
      CONFIG.repositoryRootPath,
      'apm',
  ...
  task.log(`Writing LICENSE.md to ${bundledResourcesPath}`);
...
function setAtomHelperVersion(packagedAppPath, task) {
  ...
  task.log(`Setting Atom Helper Version for ${helperPListPath}`);
...
function chmodNodeFiles(packagedAppPath, task) {
  task.log(`Changing permissions for node files in ${packagedAppPath}`);
