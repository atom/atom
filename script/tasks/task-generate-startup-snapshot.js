...
module.exports = function(packagedAppPath, task) {
  ...
  return electronLink({
    ...
    task.log('Verifying if snapshot can be executed via `mksnapshot`');
    ...
    task.log('Generating startup blob with mksnapshot');
    ...
    for (let snapshotBinary of snapshotBinaries) {
      ...
      task.log(
        `Moving generated startup blob "${snapshotBinary}" into "${destinationPath}"`
      );
      try {
