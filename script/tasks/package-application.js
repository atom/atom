...
const {Task, taskify} = require("../lib/task");

class PackageApplication extends Task {
  constructor() {
    super("Package application");
  }
  ...
  async run() {
    ...
    const packagedAppPath = await this.runPackager({
    ...
    if (process.platform === 'darwin') {
      ...
      this.setAtomHelperVersion(packagedAppPath);
    } else if (process.platform === 'linux') {
      bundledResourcesPath = path.join(packagedAppPath, 'resources');
      this.chmodNodeFiles(packagedAppPath);
    } else {
      bundledResourcesPath = path.join(packagedAppPath, 'resources');
    }

    await this.copyNonASARResources(packagedAppPath, bundledResourcesPath);

    this.info(`Application bundle created at ${packagedAppPath}`);

    return packagedAppPath;
  }

  async copyNonASARResources(packagedAppPath, bundledResourcesPath) {
    this.update(`Copying non-ASAR resources to ${bundledResourcesPath}`);
    fs.copySync(
      path.join(
        CONFIG.repositoryRootPath,
        'apm',
    ...
    this.update(`Writing LICENSE.md to ${bundledResourcesPath}`);
  ...
  setAtomHelperVersion(packagedAppPath) {
    ...
    this.update(`Setting Atom Helper Version for ${helperPListPath}`);
  ...
  chmodNodeFiles(packagedAppPath) {
    this.update(`Changing permissions for node files in ${packagedAppPath}`);
    childProcess.execSync(
      `find "${packagedAppPath}" -type f -name *.node -exec chmod a-x {} \\;`
    );
  }

  async runPackager(options) {
    ...
    return this.renamePackagedAppDir(packageOutputDirPaths[0]);
  }

  renamePackagedAppDir(packageOutputDirPath) {
    let packagedAppPath;
    if (process.platform === 'darwin') {
...
module.exports = new PackageApplication();
