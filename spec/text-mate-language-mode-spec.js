...
describe('TextMateLanguageMode', () => {
          ...
          describe('when lines are updated, but none are added or removed', () => {
            ...
            describe('when the change invalidates the tokenization of subsequent lines', () => {
              it('schedules the invalidated lines to be tokenized in the background', () => {
                ...
                buffer.insert([2, 0], '/*');
                expect(
                  languageMode.tokenizedLines[3].tokens[0].scopes
                ).toEqual(['source.js']);

                advanceClock();
                expect(
                  languageMode.tokenizedLines[3].tokens[0].scopes
                ).toEqual(['source.js', 'comment.block.js']);
                expect(
                  languageMode.tokenizedLines[4].tokens[0].scopes
                ).toEqual(['source.js', 'comment.block.js']);
                expect(
                  languageMode.tokenizedLines[5].tokens[0].scopes
                ).toEqual(['source.js', 'comment.block.js']);
              });
            });
  ...
  describe('.getFoldableRanges', () => {
    it('returns the ranges that can be folded', () => {
      ...
      expect(
        languageMode.getFoldableRanges(2).map((r) => r.toString())
      ).toEqual(
        [
